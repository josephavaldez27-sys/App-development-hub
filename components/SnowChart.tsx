...
const SnowChart: React.FC<SnowChartProps> = ({ data }) => {
  return (
    <div className="h-48 w-full mt-6 bg-black/60 p-6 rounded-sm border border-neon-cyan/10">
      <h4 className="text-[10px] font-mono text-gray-400 mb-4 uppercase tracking-[0.3em]">SNOW_WAVEFORM_ANALYSIS (IN)</h4>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
          <XAxis 
            dataKey="date" 
            tick={{fontSize: 9, fill: '#888', fontFamily: 'Fira Code'}} 
            axisLine={{stroke: '#444'}} 
            tickLine={false} 
          />
          <YAxis 
